...
import eu.esdihumboldt.mediator.context.exceptions.InconsistentContextConstraintException;

/**
 * This class contains a Envelop2D-field, id-field and no-args constractor,
...
public class Envelope2DImpl{
	 ...
	 */
	public synchronized void setEnvelope2D(Envelope2D envelope2D)throws InconsistentContextConstraintException{
		//check that the data members needed have been set 
		// if so set the method on Evelope2D
		
		if (this.envelope2D == null) throw new InconsistentContextConstraintException("SpatialConstraint:bad Envelope");
		else if (this.envelope2D.height == 0 || this.envelope2D.width == 0)throw new InconsistentContextConstraintException("SpatialConstraint:bad Envelope");
		else this.envelope2D.setFrame(this.envelope2D.x, this.envelope2D.y, this.envelope2D.width, this.envelope2D.height);
