...
public abstract class AbstractReportDefinition<T extends Report<?>, R extends T> implements ReportDefinition<T> {
	...
	public static void configureBasicReporter(Reporter<?> reporter, Properties props) throws Exception {
		...
		// taskname is set via constructor but nonetheless 
		reporter.setTaskName(props.getProperty(KEY_REPORT_TASKNAME));
		
		// TODO restore MessageType
//		reporter.setMessageType(Class.forName(props.getProperty(KEY_REPORT_MESSAGE_TYPE)););
	}
