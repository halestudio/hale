 ...
 * For instance references it uses the underlying instance collections mechanism
 * which may be inefficient.
 * The iterator supports {@link ResourceIterator#remove()} if the underlying 
 * InstanceCollection's ResourceIterator does so.
 *
...
public class MultiInstanceCollection implements InstanceCollection {
	...
	public Instance getInstance(InstanceReference reference) {
		...
		return new MultiInstanceCollectionInstance(
				collections.get(ref.listIndex).getInstance(ref.reference),
				ref.listIndex);
	}
	...
	/**
	 * Internal resource iterator iterating over all given instance collections in order.
	 * Supports {@link #remove()} if the underlying iterator supports remove.
	 *
	 * @author Kai Schwierczek
	 */
	private class MultiInstanceCollectionResourceIterator implements ResourceIterator<Instance> {
		...
		private int hasNextIndex = -1; // index of the next collection that has elements
		...
		public Instance next() {
			// check for hasNext first, because after a unsuccessful call to next(),
	...
	/**
	 * Internal class for decorating the instances with the index of the instance collection
	 * list.
	 *
	 * @author Kai Schwierczek
	 */
	...
	/**
	 * Internal class for decorating the references with the index of the instance collection
	 * list.
	 *
	 * @author Kai Schwierczek
	 */
