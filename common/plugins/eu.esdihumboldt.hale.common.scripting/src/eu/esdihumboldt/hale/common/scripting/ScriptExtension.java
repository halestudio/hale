...
public class ScriptExtension extends AbstractExtension<Script, ScriptFactory> {
	...
	/**
	 * Returns all script ids supporting the given return binding.
	 * 
	 * @param supportedReturnBinding the return binding that should be supported
	 * @return all script ids that match the given binding
	 */
	public Iterable<String> getScripts(Class<?> supportedReturnBinding) {
		Collection<String> scriptIds = new ArrayList<String>();
		for (ScriptFactory factory : getFactories()) {
			...
			if (returnBindingSupported)
				scriptIds.add(factory.getIdentifier());
		}
		return scriptIds;
	}
}
