...
public class VertexEntityTransformation implements ASTTransformation {
	...
	private MethodNode buildCreateMethod(ClassNode clazz, Expression entityName) {
		...
		// vertex
		VariableExpression vertex = new VariableExpression("vertex", VERTEX_CLASS);
		// id
		VariableExpression id = new VariableExpression("id");

		// id = OrientGraph.CLASS_PREFIX + entityName
		ExpressionStatement orientBlock = AbstractASTTransformUtil.declStatement(id,
				new BinaryExpression(new ConstantExpression(OrientGraph.CLASS_PREFIX), new Token(
						Types.PLUS, "+", -1, -1), entityName));
		// id = entityName
		ExpressionStatement normalBlock = AbstractASTTransformUtil.declStatement(id, entityName);
		...
		// > id = entityName
		// > }
		code.addStatement(new IfStatement(AbstractASTTransformUtil.isInstanceOf(graph,
				ORIENT_GRAPH_CLASS), orientBlock, normalBlock));

		// > vertex = graph.addVertex(entityName)
		Statement assignVertex = AbstractASTTransformUtil.declStatement(vertex,
				new MethodCallExpression(graph, new ConstantExpression("addVertex"),
						new ArgumentListExpression(entityName)));
