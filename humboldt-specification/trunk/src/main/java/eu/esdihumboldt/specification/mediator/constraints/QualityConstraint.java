...
/**
 * This constraint allows the definition of quality parameters that the data
 * in question will need to fulfill. For this, ISO 19113 quality definitions 
 * have been used. These are also encoded into ISO 19115 (Metadata).
 * 
...
public interface QualityConstraint extends Constraint {
	 ...
	 * QualityConstraintType; for those which are boolean, the value represents
	 * the portion of the objects that have to fulfill the QualityConstraint in 
	 * any given dataset. This portion is expressed as [0..1]. Example: If 
	 * ConceptualConsistency is checked and Tolerance is set to 0, the first 
	 * error will lead to the QualityConstraint not being satisfied. 
	 */
	 ...
	 * Allowable Values: [0..1]</li>
	 * <li>mean: the mean absolute error that may occur.
	 * Allowable Values: unrestricted</li>
	 * </ul>
	 */
	public enum ToleranceType {
		/**
		 * The absolute maximum numeric error that may occur. 
		 * Allowable Values: unrestricted
		 */
		absolute,
		/**
		 * The maximum relative (local) error that may occur.
		...
		portion,
		/**
		 * mean: the mean absolute error that may occur.
	     * Allowable Values: unrestricted
		 */
		mean
	}
