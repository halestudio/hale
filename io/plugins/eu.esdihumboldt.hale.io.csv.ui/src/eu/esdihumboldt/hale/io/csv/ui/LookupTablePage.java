...
public class LookupTablePage extends LookupTableImportConfigurationPage implements
	...
	public boolean updateConfiguration(LookupTableImport provider) {
		if (skip) {
			provider.setParameter(CSVLookupReader.PARAM_SKIP_FIRST_LINE, Value.of("True"));
		}
		else {
			provider.setParameter(CSVLookupReader.PARAM_SKIP_FIRST_LINE, Value.of("False"));
		}
		if (fColumn.getSelectionIndex() != -1 && sColumn.getSelectionIndex() != -1) {
			provider.setParameter(CSVLookupReader.FIRST_COLUMN,
					Value.of(fColumn.getSelectionIndex()));
			provider.setParameter(CSVLookupReader.SECOND_COLUMN,
					Value.of(sColumn.getSelectionIndex()));
			return true;
		}
		else {
