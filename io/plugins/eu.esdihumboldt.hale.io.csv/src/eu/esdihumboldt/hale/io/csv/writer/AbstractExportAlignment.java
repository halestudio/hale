...
public abstract class AbstractExportAlignment extends AbstractAlignmentWriter {
	 ...
	 */
	public final List<String> header = Arrays.asList("Source type", "Source type conditions",
			"Source properties", "Source property conditions", "Target type", "Target properties",
			"Relation name", "Cell explanation", "Cell notes");
	 ...
	 */
	public List<Map<CellType, CellInfo>> getMappingData() {
	...
	/**
	 * @param mapping the relevant mapping of {@link #getMappingData()}
	...
	public String getCellValue(Map<CellType, CellInfo> mapping, CellType cellType) {
		...
		case SOURCE_PROPERTY_CONDITIONS:
			type = CellType.SOURCE_PROPERTIES;
			break;
		case TARGET_TYPE:
			type = CellType.TARGET_PROPERTIES;
			break;
		...
		case TARGET_PROPERTIES:
			int highestHierarchyLevel = 100;
			CellInfo cellInfo = mapping.get(cellType);
			List<String> text = cellInfo.getText();
			List<Integer> hierarchyLevels = cellInfo.getHierarchyLevels();
			for (int i = 0; i < cellInfo.getText().size(); i++) {
				String info = text.get(i);
				int hierarchyLevel = hierarchyLevels.get(i);
				if (!init) {
					result.append(info);
					init = true;
				}
				else {
					if (highestHierarchyLevel == hierarchyLevel) {
						// sub properties are signed with a point
						info = "." + info;
					}
					else if (hierarchyLevel > highestHierarchyLevel) {
						info = CSVWriter.DEFAULT_LINE_END + info;
					}
					result.append(CSVWriter.DEFAULT_LINE_END);
					result.append(info);
				}
				highestHierarchyLevel = hierarchyLevel;
			}
		...
		case CELL_NOTES:
			...
			for (String info : mapping.get(cellType).getText()) {
				if (!init) {
					result.append(info);
					init = true;
				}
		...
		// in case of SOURCE_TYPE, SORUCE_PROPERTY_CONDITIONS or TARGET_TYPE
		CellInfo conditions = mapping.get(cellType);
		List<Integer> propertiesHierarchy = mapping.get(type).getHierarchyLevels();
		List<String> text = conditions.getText();
		List<Integer> topVariables = conditions.getHierarchyLevels();

		if (text.size() > 0) {
			result.append(text.get(0));
			// if text size is 1, we possibly need line breaks after the text
			if (text.size() == 1) {
				int zeroHierarchyLevel = topVariables.get(0);
				for (int level = 0; level < propertiesHierarchy.size(); level++) {
					int propHierarchy = propertiesHierarchy.get(level);
					result.append(CSVWriter.DEFAULT_LINE_END);
					if (zeroHierarchyLevel <= propHierarchy) {
						break;
					}
				}
			}
				...
				for (int i = 1; i < text.size(); i++) {
					String info = text.get(i);
					int currentHierarchyLevel = topVariables.get(i);
					boolean infoSet = false;
					for (int level = 0; level < propertiesHierarchy.size(); level++) {
						int propHierarchy = propertiesHierarchy.get(level);
						result.append(CSVWriter.DEFAULT_LINE_END);
						if (currentHierarchyLevel <= propHierarchy && !infoSet) {
							result.append(info);
							infoSet = true;
						}
					}
	...
	private void addCellData(Cell cell) {
		...
		if (cell.getSource() != null) {
			...
			for (Entity entity : cell.getSource().values()) {
				// column source type
				sourceType.addText(entity.getDefinition().getType().getDisplayName());
				sourceType.addHierarchyLevel(hierarchyLevel);

				// column source type conditions
				Filter entityFilter;
				if ((entityFilter = entity.getDefinition().getFilter()) != null) {
					sourceTypeConditions.addText(FilterDefinitionManager.getInstance().asString(
							entityFilter));
					sourceTypeConditions.addHierarchyLevel(hierarchyLevel);
				}
				...
				for (ChildContext childContext : entity.getDefinition().getPropertyPath()) {
					...
					if (child != null) {
						...
						sourceProperties.addText(child.getDisplayName());
						sourceProperties.addHierarchyLevel(hierarchyLevel);
					}
					...
					if (childContext.getCondition() != null) {
						...
						sourcePropertyConditions.addText(FilterDefinitionManager.getInstance()
								.asString(contextFilter));
						sourcePropertyConditions.addHierarchyLevel(hierarchyLevel);
					}
				}
				// next entries must have higher hierarchy level
				hierarchyLevel++;
			}
		}
		if (cell.getTarget() != null) {
			...
			for (Entity entity : cell.getTarget().values()) {
				// column target type
				targetType.addText(entity.getDefinition().getType().getDisplayName());
				targetType.addHierarchyLevel(hierarchy);

				// column target properties
				for (ChildContext childContext : entity.getDefinition().getPropertyPath()) {
					PropertyDefinition child = childContext.getChild().asProperty();
					if (child != null) {
						targetProperties.addText(child.getDisplayName());
						targetProperties.addHierarchyLevel(hierarchy);
					}
				}
			}
			hierarchy++;
		...
		relationName.addText(function.getDisplayName());

		// column cell explanation
		CellExplanation cellExpl = function.getExplanation();
		if (cellExpl != null) {
			cellExplanation.addText(function.getExplanation().getExplanation(cell,
