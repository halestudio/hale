...
//		allAllowedClasses.add(InstanceAccessor.class);
	...
	private Set<Class<?>> instanceAllowedClasses = new HashSet<>(allowedClasses);
	private Set<Class<?>> instanceAllAllowedClasses = new HashSet<>(allAllowedClasses);

	/**
	 * Constructor using additional allowed classes.
	 *
	 * @param additionalAllowedClasses classes, which may be initialized, and all their declared methods may be used
	 * @param additionalAllAllowedClasses classes, which may be initialized, and any call on them is allowed (has to implement methodMissing or equal)
	 */
	public RestrictiveGroovyInterceptor(Set<Class<?>> additionalAllowedClasses, Set<Class<?>> additionalAllAllowedClasses) {
	...
	private boolean checkMethodCall(Object receiver, String method) throws GroovyRestrictionException {
