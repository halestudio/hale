 ...
 */
public class NetworkExpansionExplanation extends AbstractCellExplanation implements NetworkExpansionFunction {
	...
	private static final String EXPLANATION_PATTERN = "Takes a geometry found in the {0} property and creates a buffer geometry. The buffer geometry is assigned to the {1} property in the target type.\n" + 
			...
			"{2}\n" +
			"Source property variables int the expression are replaced by the corresponding property value, if the context condition/index matches, otherwise the value isn't set.";
	...
	protected String getExplanation(Cell cell, boolean html) {
		...
		if (target != null && expression != null) {
			...
			String explanation = MessageFormat.format(EXPLANATION_PATTERN, 
					formatEntity(geom.get(0), html, true), 
					formatEntity(target, html, true), expression);
			if (html)
				explanation = explanation.replaceAll("\n", "<br />");
			if (html) {
				StringBuilder sb = new StringBuilder(); //TODO unify the replacement tables by introducing a common method to produce them
