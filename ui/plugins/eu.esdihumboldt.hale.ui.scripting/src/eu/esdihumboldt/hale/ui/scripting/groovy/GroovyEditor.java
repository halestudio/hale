...
public class GroovyEditor extends AbstractEditor<String> {
	...
	public GroovyEditor(Composite parent, Script script, Class<?> binding) {
		...
		composite.setLayout(GridLayoutFactory.swtDefaults().extendedMargins(5, 0, 0, 0).create());
		...
		// control decoration
		final ControlDecoration decorator = new ControlDecoration(viewer.getControl(), SWT.LEFT
				| SWT.TOP, composite);
		// set initial status
		decorator.hide();
		// set image
		...
		viewer.getDocument().addDocumentListener(new IDocumentListener() {
			...
			public void documentChanged(DocumentEvent event) {
				...
				// validate
				String result = GroovyEditor.this.script.validate(currentValue,
