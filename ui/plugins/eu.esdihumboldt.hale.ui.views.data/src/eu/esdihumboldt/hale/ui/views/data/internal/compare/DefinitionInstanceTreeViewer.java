...
public class DefinitionInstanceTreeViewer implements InstanceViewer {
	...
	public void createControls(final Composite parent) {
		...
		treeViewer = new TreeViewer(main, SWT.SINGLE | SWT.FULL_SELECTION
				| SWT.BORDER);

		treeViewer.setContentProvider(new TypeMetaPairContentProvider(
				treeViewer));
		...
		treeViewer.getTree().addMouseMoveListener(new MouseMoveListener() {
			...
			public void mouseMove(MouseEvent e) {
				...
				if (cell == null
						...
						|| (editor.getItem() == cell.getItem() && editor
				...
				String pattern = "\\Q"
						+ DefinitionInstanceLabelProvider.MULTIPLE_VALUE_FORMAT
						+ "\\E$";
				pattern = pattern.replace("{0}", "\\E(\\d+)\\Q").replace("{1}",
						"\\E(\\d+)\\Q");
						...
						if (event.getSelection() instanceof IStructuredSelection) {
							...
							labelProviders
									...
									.selectPath(
											...
											(Integer) (((IStructuredSelection) event
													.getSelection())
													.getFirstElement()));
		...
		treeViewer.getTree().addMouseMoveListener(new MouseMoveListener() {
			...
			public void mouseMove(MouseEvent e) {
				...
				if (cell == null
						|| cell.getColumnIndex() == 0
						|| (metaEditor.getItem() == cell.getItem() && metaEditor
								.getColumn() == cell.getColumnIndex())) {
					return;
				}
				
				//Initiate the extensionpoint
				MetadataActionExtension mae = MetadataActionExtension
						.getInstance();
				//get all defined actions
				final List<MetadataAction> actions = mae.getMetadataAction(cell
						.getElement().toString());
				...
				//Toolbar used to view and trigger the different possible actions
				ToolBar tooli = new ToolBar(treeViewer.getTree(), SWT.NONE);			
				for (final MetadataAction act : actions) {
					ToolItem action = new ToolItem(tooli, SWT.PUSH);
					action.setText(act.getText());
					action.addSelectionListener(new SelectionAdapter() {
						
						@Override
						public void widgetSelected(SelectionEvent e) {

							act.execute(key, value);
						}
					});
				}
				metaEditor.setEditor(tooli, (TreeItem) cell.getItem(),
						cell.getColumnIndex());
				tooli.pack();
			}
		});
	...
	public void setInput(TypeDefinition type, Iterable<Instance> instances) {
		...
		//
		// if (displayLineage) {
		// Object[][] processStepsText = new Object[lineageLength][featuresSize
		// + 1];
		// int featureIndex = 0;
		// for (Feature f : features) {
		//						Lineage l = (Lineage) f.getUserData().get("METADATA_LINEAGE"); //$NON-NLS-1$
		...
		// processStepsText[psIndex][featureIndex + 1] =
		// ps.getDescription().toString();
		// psIndex++;
		// }
		// }
		// featureIndex++;
		...
		// DefaultTreeNode processStep = new
		// DefaultTreeNode(processStepsText[i]);
		...
		if (type != null) {
			...
			Pair<TypeDefinition, Set<String>> pair = new Pair<TypeDefinition, Set<String>>(
					type, completeMetaNames);
		...
		if (instances != null) {
			...
			for (Instance instance : instances) { // sortedFeatures) {
				final TreeViewerColumn column = new TreeViewerColumn(
						treeViewer, SWT.LEFT);
				...
				if (layout instanceof TreeColumnLayout) {
					((TreeColumnLayout) layout).setColumnData(
							column.getColumn(), new ColumnWeightData(1));
				}

				// add tool tip
				// new ColumnBrowserTip(treeViewer, 400, 300, true, index,
				// null);
