...
public class RuleStylePage extends FeatureStylePage {
	...
	public RuleStylePage(FeatureStyleDialog parent) {
		...
		if (addImage == null) {
			addImage = InstanceStylePlugin.getImageDescriptor(
					"/icons/add.gif").createImage(); //$NON-NLS-1$
		}
		
		if (removeImage == null) {
			removeImage = InstanceStylePlugin.getImageDescriptor(
					"/icons/remove.gif").createImage(); //$NON-NLS-1$
		}
		
		if (renameImage == null) {
			renameImage = InstanceStylePlugin.getImageDescriptor(
					"/icons/rename.gif").createImage(); //$NON-NLS-1$
		}
		
		if (upImage == null) {
			upImage = InstanceStylePlugin.getImageDescriptor(
					"/icons/arrow_up.gif").createImage(); //$NON-NLS-1$
		}
		
		if (downImage == null) {
			downImage = InstanceStylePlugin.getImageDescriptor(
					"/icons/arrow_down.gif").createImage(); //$NON-NLS-1$
		}
	}
