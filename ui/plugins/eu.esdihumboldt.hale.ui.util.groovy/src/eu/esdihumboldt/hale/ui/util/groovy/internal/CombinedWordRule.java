...
public class CombinedWordRule implements IRule {
	...
	public static class WordMatcher {
		...
		private final Map<CharacterBuffer, IToken> fWords = new HashMap<CharacterBuffer, IToken>();

		/**
		 * Adds a word and the token to be returned if it is detected.
		 * 
		 * @param word the word this rule will search for, may not be
		 *            <code>null</code>
		 * @param token the token to be returned if the word has been found, may
	...
	private final CharacterBuffer fBuffer = new CharacterBuffer(16);

	/** List of word matchers */
	private final List<WordMatcher> fMatchers = new ArrayList<WordMatcher>();
	 ...
	 * @param detector the word detector to be used by this rule, may not be
	 *            <code>null</code>
	 * @param defaultToken the default token to be returned on success if
	 *            nothing else is specified, may not be <code>null</code>
	 * 
	 * @see WordMatcher#addWord(String, IToken)
	 */
