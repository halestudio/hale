...
public class PreferencesGroovyService extends DefaultGroovyService {
	...
	/**
	 * @see eu.esdihumboldt.util.groovy.sandbox.DefaultGroovyService#evaluate(groovy.lang.Script)
	 */
	@Override
				...
				public void run() {
					boolean result = MessageDialog
							.openQuestion(
									...
									"Groovy script restriction",
									"A Groovy script tries using a restricted method, do you want to lift all restrictions?\n\nWARNING: The Groovy script can then do \"anything\", so be sure to trust your source!");
	...
	private synchronized void updateRestrictionActive() {
		...
		if (!Objects.equal(location, restrictionActiveURI)) {
			if (location == null) {
				...
				// XXX add feedback or ask user whether to keep setting in case
				// of present, but outdated hash?
				// XXX do the above on project load?
				restrictionActive = !hashChecked;
			}
			restrictionActiveURI = location;
			askedForAllowance = false; // TODO delete this, when saving a
										// project saves preferences
		}
	}
