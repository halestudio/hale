...
class ExecuteTest extends GroovyTestCase {
	...
	private int transform(List<String> args = [], Closure exec) {
		...
		try {
			...
			allArgs.add(0, '-application')
			allArgs.add(1, 'hale.transform')

			console.println ">> Executing application with the following arguments: ${allArgs.join(' ')}"

			//			res = new Main().run(allArgs as String[])
			res = ApplicationUtil.launchApplication('hale.transform', args)
		} finally {
