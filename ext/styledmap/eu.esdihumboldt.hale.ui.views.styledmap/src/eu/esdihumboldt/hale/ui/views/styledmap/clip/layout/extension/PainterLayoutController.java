...
public class PainterLayoutController {
	...
	public PainterLayoutController(BasicMapKit mapKit) {
		...
		pls = (PainterLayoutService) PlatformUI.getWorkbench().getService(PainterLayoutService.class);
	 ...
	 */
	private void applyLayout(PainterLayoutFactory currentDefinition,
			PainterLayout current) {
		...
		for (int i = 0; i < painters.size(); i++) {
			...
			if (i >= clips.size()) {
				...
				painter.enable();
				painter.setClip(clip); //XXX is this working even if the painter is not enabled (in time)?
			}
		}
