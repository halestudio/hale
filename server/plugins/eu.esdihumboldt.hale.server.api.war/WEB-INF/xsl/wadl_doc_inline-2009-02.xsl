...
>
                ...
                <ul>
                    ...
                    <xsl:if test="descendant::wadl:fault">
                        <li><a href="#faults">Faults</a>
                            <ul>
                                <xsl:apply-templates select="wadl:resources/descendant::wadl:fault" mode="toc"/>
                            </ul>
                        </li>
                    </xsl:if>
                </ul>
    ...
    <xsl:template name="get-namespace-uri">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="wadl:resources/*[1]/attribute::*[namespace-uri()='urn:namespace' and local-name()=$prefix]"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>

    <xsl:template name="get-element">
        <!-- removed node-set call -->
    </xsl:template>
