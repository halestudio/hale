...
package eu.esdihumboldt.hale.ui.wizards.functions.contribution;
...
import eu.esdihumboldt.hale.ui.wizards.functions.contribution.internal.SchemaSelectionWizardAction;
...
public class SchemaSelectionFunctionContribution extends
	...
	private boolean matchPropertyFunction(PropertyFunction function,
		...
		// check counts
		if (!checkCount(sourceItems.size(), function.getSourceProperties())) {
			return false;
		}
		if (!checkCount(targetItems.size(), function.getTargetProperties())) {
			return false;
		}
	...
	private boolean matchTypeFunction(TypeFunction function, SchemaSelection selection) {
		...
		// check counts
		if (!checkCount(sourceItems.size(), function.getSourceTypes())) {
			return false;
		}
		if (!checkCount(targetItems.size(), function.getTargetTypes())) {
			return false;
		}
