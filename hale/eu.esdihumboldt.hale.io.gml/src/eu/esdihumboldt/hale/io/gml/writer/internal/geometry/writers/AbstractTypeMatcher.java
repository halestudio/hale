...
/**
 * Abstract pattern based type matcher
 *
...
public abstract class AbstractTypeMatcher {
	...
	private static final ALogger log = ALoggerFactory.getLogger(AbstractTypeMatcher.class);
			...
			if (path != null) {
				...
				if (verifyPatterns != null && !verifyPatterns.isEmpty()) {
					for (Pattern verPattern : verifyPatterns) {
						DefinitionPath endPoint = verPattern.match(path.getLastType(), new DefinitionPath(path), defaultNs);
						if (endPoint != null) {
							// verify end-point
